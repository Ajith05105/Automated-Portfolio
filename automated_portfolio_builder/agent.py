...
concurrent callers. build_root_agent() returns a fresh agent tree with
fresh McpToolset instances on every call so each attendee session gets
its own subprocess.
"""
...
def build_root_agent() -> SequentialAgent:
    """Builds a fresh portfolio pipeline with isolated MCP subprocesses.

    Call once per attendee session — each call spawns its own Drive and
    Netlify stdio subprocesses, avoiding response misrouting when multiple
    ...
    delivery_agent = LlmAgent(
        ...
        3. Your final response must be EXACTLY one word — either sent or failed.
